...
static inline int msc_send(usbd_mass_storage *ms, int len, const void *data)
{
    ...
    len = usbd_ep_write_packet(ms->usbd_dev, ms->ep_in, data, len);
    //console_log("send: %d\r\n", len);
...
static void msc_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    console_log("state: %d\r\n", ms->state);
    console_log("recv: %d byte, cached: %d\r\n", len, ms->cache_cnt);
